...
- (void)didOpen {
    ...
    ispy_log("Opened new Shell WebSocket connection. Launching %s", [[self cmdLine] UTF8String]);
...
- (void)didReceiveMessage:(NSString *)msg {
    //ispy_log("WebSocket Shell message: %s", [msg UTF8String]);
...
// callback for CocoaHTTPServer WebSocket class
-(void)didClose {
    ispy_log("WebSocket Shell connection closed, waiting...");
    int info;
    ...
    ispy_log("Zombies reaped. Closing...");
...
-(void) pipeDataToWebsocket {
    ...
    ispy_log("pipeDataToWebsocket from master PTY %d", fdMasterPTY);
