...
- (HTTPMessage *)newUniRangeResponse:(UInt64)contentLength
{
	...
	NSString *contentLengthStr = [NSString stringWithFormat:@"%llu", range.length];
	[response setHeaderField:@"Content-Length" value:contentLengthStr];
	
	NSString *rangeStr = [NSString stringWithFormat:@"%llu-%llu", range.location, DDMaxRange(range) - 1];
	NSString *contentRangeStr = [NSString stringWithFormat:@"bytes %@/%llu", rangeStr, contentLength];
...
- (HTTPMessage *)newMultiRangeResponse:(UInt64)contentLength
{
	...
	for (i = 0; i < [ranges count]; i++)
	{
		...
		NSString *rangeStr = [NSString stringWithFormat:@"%llu-%llu", range.location, DDMaxRange(range) - 1];
		NSString *contentRangeVal = [NSString stringWithFormat:@"bytes %@/%llu", rangeStr, contentLength];
	...
	NSString *contentLengthStr = [NSString stringWithFormat:@"%llu", actualContentLength];
	[response setHeaderField:@"Content-Length" value:contentLengthStr];
...
- (void)sendResponseHeadersAndBody
{
	...
	if (!isRangeRequest)
	{
		...
		{
			NSString *contentLengthStr = [NSString stringWithFormat:@"%llu", contentLength];
			[response setHeaderField:@"Content-Length" value:contentLengthStr];
		}
	}
	else
...
- (NSData *)preprocessErrorResponse:(HTTPMessage *)response
{
	...
	//     
	//     NSString *contentLengthStr = [NSString stringWithFormat:@"%llu", (unsigned long)[msgData length]];
	//     [response setHeaderField:@"Content-Length" value:contentLengthStr];
	// }
