...
private:
	class Type {
		...
		bool is_compatible_with(const Type& another, const ObjCTypeRecord& record, std::unordered_set<TypePointerPair>& banned_pairs) const throw();
	...
	std::unordered_map<std::string, TypeIndex> ma_indexed_types;
	std::vector<Type> ma_type_store;
	
	std::unordered_map<TypeIndex, std::unordered_map<TypeIndex, EdgeStrength> > ma_adjlist;
	std::unordered_map<TypeIndex, unsigned> ma_k_in, ma_strong_k_in;
...
public:
	...
	const std::unordered_map<TypeIndex, EdgeStrength>* dependencies(TypeIndex type_index) const throw() { 
		std::unordered_map<TypeIndex, std::unordered_map<TypeIndex, EdgeStrength> >::const_iterator cit = ma_adjlist.find(type_index);
	...
	EdgeStrength link_strength(TypeIndex a, TypeIndex b) const throw() {
		std::unordered_map<TypeIndex, std::unordered_map<TypeIndex, EdgeStrength> >::const_iterator ait = ma_adjlist.find(a);
		if (ait == ma_adjlist.end())
			return ES_None;
		else {
			std::unordered_map<TypeIndex, EdgeStrength>::const_iterator bit = ait->second.find(b);
	...
	unsigned link_count(TypeIndex type_index, bool strong_only = false) const throw() { 
		std::unordered_map<TypeIndex, unsigned>::const_iterator cit = (strong_only ? ma_strong_k_in : ma_k_in).find(type_index);
